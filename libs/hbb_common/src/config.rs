...
lazy_static::lazy_static! {
    ...
    // SinBuque. Este nombre no es decorativo: de acá salen el título de la ventana, el
    // nombre del servicio de Windows, las carpetas de configuración y las de log.
    pub static ref APP_NAME: RwLock<String> = RwLock::new("SinBuque".to_owned());
    static ref KEY_PAIR: Mutex<Option<KeyPair>> = Default::default();
    static ref USER_DEFAULT_CONFIG: RwLock<(UserDefaultConfig, Instant)> = RwLock::new((UserDefaultConfig::load(), Instant::now()));
    pub static ref NEW_STORED_PEER_CONFIG: Mutex<HashSet<String>> = Default::default();
    pub static ref DEFAULT_SETTINGS: RwLock<HashMap<String, String>> = Default::default();
    /// Ajustes que SinBuque **impone**: no se pueden cambiar desde la interfaz ni guardar
    /// por encima.
    ///
    /// Es distinto de `BUILTIN_SETTINGS`, que son valores de entrada: esto es lo que no
    /// queremos que nadie apague sin querer.
    pub static ref OVERWRITE_SETTINGS: RwLock<HashMap<String, String>> = RwLock::new(HashMap::from([
        // **La sesión no se corta sola, nunca.** Estas máquinas se manejan durante jornadas
        // enteras y una desconexión «por inactividad» a las tres de la mañana significa que
        // alguien tiene que estar físicamente ahí para volver a levantar todo. Es exactamente
    ...
    /// soportada de cambiar cómo se comporta de entrada, sin andar tocando pantallas una por
    /// una. En RustDesk se llena desde un archivo firmado con la clave de ellos, que no
    /// tenemos: por eso va compilada.
    pub static ref BUILTIN_SETTINGS: RwLock<HashMap<String, String>> = RwLock::new(HashMap::from([
        // El «powered by RustDesk» del pie. No es desprecio: la atribución está donde
...
// Nuestro servidor, compilado adentro. Es lo que hace que el programa ande contra SinBuque
// recién instalado, sin que nadie tenga que configurar nada ni renombrar el archivo.
pub const RENDEZVOUS_SERVERS: &[&str] = &["sinbuque.online"];
// La clave pública de NUESTRO servidor (`/opt/sinbuque/data/id_ed25519.pub`). Sin esto el
// cliente se conectaría igual, pero aceptaría a cualquiera que se haga pasar por el servidor.
pub const RS_PUB_KEY: &str = "c6KGFNChs8lTd71syukdL5G170747WcoHjrn5Y1Mvjo=";
...
pub fn is_disable_ab() -> bool {
    // En SinBuque la libreta de direcciones **no existe**: vive en la nube de RustDesk, que no
    // usamos. Se apaga por la misma palanca que el programa ya consulta en todos lados, así
    // que desaparece de la interfaz entera sin tocar una sola pantalla.
    true
}

#[inline]
pub fn is_disable_account() -> bool {
    // Ídem la cuenta: sin su nube no hace nada, y un botón de «iniciar sesión» que no lleva a
